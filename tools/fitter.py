        ...
        for sf in sfs:
          for term in sf:
            ...
            for poi in term.split("_")[1:]:
              ...
              if poi not in list(self.PTerms.keys()): self.PTerms[poi] = 0.
  ...
  def __init__(self,inputMeasurement,functions,doAsimov):
    ...
    for ix in inputMeasurement['X']:
      for jx in inputMeasurement['X']:
        if (ix,jx) in list(inputMeasurement['rho'].keys()): p = inputMeasurement['rho'][(ix,jx)]
        elif (jx,ix) in list(inputMeasurement['rho'].keys()): p = inputMeasurement['rho'][(jx,ix)]
        else:
          if ix == jx: p = 1.
          else: 
            p = 0.
    ...
    nbins = len( list(inputMeasurement['X'].keys()) )
